...
class Window(QWidget):
        ...
        def show():
            ...
            for i in range(len(received)):
                if received[i] == "":
                    msg = (
                        str(sent[i])
                        .replace(f"{sent[i].source} -> {sent[i].destination} : ", "")
                        .replace(f"{sent[i].source}->{sent[i].destination} : ", "")
                    )
                    ...
                    msg = (
                        str(received[i])
                        .replace(
                            f"{received[i].source} -> {received[i].destination} : ", ""
                        )
                        .replace(
                            f"{received[i].source}->{received[i].destination} : ", ""
                        )
                    ...
                    content.append(
                        [
                            str(received[i].source),
                            received_msg,
                            str(sent[i].destination),
                            sent_msg,
                        ]
                    )
    ...
    def show_all_data(self):
        ...
        content = [
            [
                str(messages[i].source),
                str(messages[i].destination),
                message_content[i],
                str(i),
            ]
        ...
        table = MyTable(content, title="All data")
        self.windows.append(table)
    ...
    def show_queue(self):
        ...
        for messages in queue:
            for message in messages:
                message_stripped = (
                    str(message)
                    .replace(f"{message.source} -> {message.destination} : ", "")
                    .replace(f"{message.source}->{message.destination} : ", "")
                )
                content.append(
                    [str(message.source), str(message.destination), message_stripped]
                )
        parent = self
    ...
    def pick(self):
        ...
        def execute(device, index):
            def inner_execute():
                if self.emulator._devices[device]._finished:
                    table.destroy(True, True)
                    print(
                        f"{RED}The selected device has already finished execution!{RESET}"
                    )
                    return
                if self.emulator.parent is AsyncEmulator:
    ...
    def set_device_color(self):
        sleep(0.1)
        messages = (
            self.emulator.messages_sent
            if self.emulator.last_action == "send"
            else self.emulator.messages_received
        )
        if len(messages) != 0:
            ...
            if not last_message == self.last_message:
                ...
                if last_message.source == last_message.destination:
                    self.buttons[last_message.source].setStyleSheet(
                        circle_button_style(self.device_size, "yellow")
                    )
                else:
                    self.buttons[last_message.source].setStyleSheet(
                        circle_button_style(self.device_size, "green")
                    )
                    self.buttons[last_message.destination].setStyleSheet(
                        circle_button_style(self.device_size, "red")
                    )
    ...
    def main(self, num_devices, restart_function):
        ...
        for i in range(num_devices):
            x, y = self.coordinates(
                (device_area.width() / 2, device_area.height() / 2),
                (device_area.height() / 2) - (self.device_size / 2),
                i,
                num_devices,
            )
        ...
        button_actions = {
            "Step": self.step,
            "End": self.end,
            "Restart algorithm": lambda: self.restart_algorithm(restart_function),
            "Show all messages": self.show_all_data,
            "Switch emulator": self.swap_emulator,
