...
def run_exercise(
    ...
    if number_of_devices < 2:
        raise IndexError(
            f"At least two devices are needed as an input argument, got {number_of_devices}"
        )
    emulator = None
    ...
    if isinstance(instance, SteppingEmulator) and gui:
        window = Window(
            number_of_devices,
            lambda: run_exercise(
                lecture_no, algorithm, network_type, number_of_devices, True
            ),
            instance,
        )
        window.show()
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="For exercises in Distributed Systems."
    )
    parser.add_argument(
        "--lecture",
        metavar="N",
        ...
        help="Lecture number",
        required=True,
        choices=[0, 1, 2, 4, 5, 6, 7, 8, 9, 10, 11, 12],
    )
    parser.add_argument(
        "--algorithm",
        metavar="alg",
        type=str,
        nargs=1,
        help="Which algorithm from the exercise to run",
        required=True,
    )
    parser.add_argument(
        "--type",
        ...
        type=str,
        nargs=1,
        help="whether to use [async] or [sync] network",
        required=True,
        choices=["async", "sync", "stepping"],
    )
    parser.add_argument(
        "--devices",
        metavar="N",
        type=int,
        nargs=1,
        help="Number of devices to run",
        required=True,
    )
    parser.add_argument(
        "--gui", action="store_true", help="Toggle the gui or cli", required=False
    )
    ...
    if args.gui and args.type[0] == "stepping":
        ...
        run_exercise(
            args.lecture[0], args.algorithm[0], args.type[0], args.devices[0], True
        )
        app.exec()
    else:
        run_exercise(
            args.lecture[0], args.algorithm[0], args.type[0], args.devices[0], False
        )
