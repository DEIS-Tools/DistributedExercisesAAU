...
class BasicMulticast(Device):

    def __init__(self, index: int, number_of_devices: int, medium: Medium, application: MulticastListener):
        super().__init__(index, number_of_devices, medium)
        self._application = application
        self._outbox = []
        self._group = medium.ids()
...
class ReliableIPMulticast(MulticastListener,MulticastService,Device):
    ...
    def forward(self, message):
        if isinstance(message, NACK):
            self.medium().send((self.index(), self._seq_numbers,
                                self._received[(self.index(), message.seq_number)]))
        else:
            self._application.forward(message)

    def try_deliver(self):
        for (oid, seqnr), content in self._received:
            if self._seq_numbers[oid] == seqnr:
                self._application.deliver((oid, seqnr, content))
...
class ISISMulticast(MulticastListener,MulticastService,Device):
    ...
    def __init__(self, index: int, number_of_devices: int, medium: Medium, application: MulticastListener):
        ...
        self._a_seq = 0 # last agreed
        self._p_seq = 0 # last proposed
    ...
    def forward(self, message):
        if isinstance(message, Vote):
            self._votes[message.message_id()].append(message.order())
            votes = self._votes
